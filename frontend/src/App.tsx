...
function Shell({user, page, setPage, onLogout, children}: {user: SessionUser; page: PageName; setPage: (p: PageName) => void; onLogout: () => void; children: ReactNode}) {
  ...
  return <div className="app-shell"><aside className={open ? "sidebar open" : "sidebar"}><div className="sidebar-brand"><div className="brand-mark"><Scissors/></div><div><strong>M&amp;R BarberHub</strong><small>Gestão de barbearias</small></div><button className="icon-btn mobile-only" onClick={() => setOpen(false)}><X/></button></div><nav>{navItems.filter(i => !i.admin || user.role === "ADMIN").map(item => <button key={item.page} className={page === item.page ? "active" : ""} onClick={() => {setPage(item.page); setOpen(false);}}><item.icon/>{item.label}</button>)}</nav><div className="sidebar-user"><div className="avatar">{(user.name || "A").charAt(0)}</div><div><strong>{user.name || "Administrador"}</strong><small>{user.role === "ADMIN" ? "Administrador" : "Funcionário"}</small></div><button title="Sair" className="icon-btn" onClick={onLogout}><LogOut/></button></div></aside><main className="content"><button className="mobile-menu icon-btn" onClick={() => setOpen(true)}><Menu/></button>{children}</main></div>;
}
