...
import {
  CalendarDays, CalendarX, Clock3, LayoutDashboard, LogOut, Menu, Plus, Save,
...
function LoginPage({onLogin}: {onLogin: (user: SessionUser) => void}) {
  ...
  return <main className="login-shell"><section className="login-copy"><div className="brand-mark"><Scissors/></div><p className="eyebrow">SaaS dos Bigodes</p><h1>Sua barbearia organizada.<br/><span>Seu tempo de volta.</span></h1><p>Agenda, clientes, equipe e serviços em um painel simples de usar.</p><p className="company-credit">Um produto <strong>M&amp;R Solutions</strong></p></section><section className="login-card"><h2>Bem-vindo</h2><p>Entre para administrar sua barbearia.</p><form onSubmit={e => {e.preventDefault(); login.mutate();}}><label>Usuário<input autoFocus value={username} onChange={e => setUsername(e.target.value)}/></label><label>Senha<input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Sua senha"/></label><ErrorMessage error={login.error}/><Button disabled={!username || password.length < 8 || login.isPending}>{login.isPending ? "Entrando..." : "Entrar"}</Button></form><p className="company-credit login-credit">Tecnologia por <strong>M&amp;R Solutions</strong></p></section></main>;
}
...
function Shell({user, page, setPage, onLogout, children}: {user: SessionUser; page: PageName; setPage: (p: PageName) => void; onLogout: () => void; children: ReactNode}) {
  ...
  return <div className="app-shell"><aside className={open ? "sidebar open" : "sidebar"}><div className="sidebar-brand"><div className="brand-mark"><Scissors/></div><div><strong>Bigodes</strong><small>Gestão de barbearia</small></div><button className="icon-btn mobile-only" onClick={() => setOpen(false)}><X/></button></div><nav>{navItems.filter(i => !i.admin || user.role === "ADMIN").map(item => <button key={item.page} className={page === item.page ? "active" : ""} onClick={() => {setPage(item.page); setOpen(false);}}><item.icon/>{item.label}</button>)}</nav><div className="sidebar-user"><div className="avatar">{(user.name || "A").charAt(0)}</div><div><strong>{user.name || "Administrador"}</strong><small>{user.role === "ADMIN" ? "Administrador" : "Funcionário"}</small></div><button title="Sair" className="icon-btn" onClick={onLogout}><LogOut/></button></div></aside><main className="content"><button className="mobile-menu icon-btn" onClick={() => setOpen(true)}><Menu/></button>{children}</main></div>;
}
...
function PublicBooking({slug}:{slug:string}) {
  const [service,setService]=useState(""); const [day,setDay]=useState(format(new Date(),"yyyy-MM-dd")); const [slot,setSlot]=useState(""); const [name,setName]=useState(""); const [whatsapp,setWhatsapp]=useState(""); const [captcha,setCaptcha]=useState(import.meta.env.VITE_TURNSTILE_SITE_KEY ? "" : "development"); const captchaRef=useRef<HTMLDivElement>(null);
  ...
  const booking=useMutation({mutationFn:()=>api.post(`/public/${slug}/book/`,{name,whatsapp,service_id:Number(service),starts_at:slot,captcha_token:captcha})});
  useEffect(()=>{const sitekey=import.meta.env.VITE_TURNSTILE_SITE_KEY;if(!sitekey)return;const script=document.createElement("script");script.src="https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit";script.async=true;script.onload=()=>{if(captchaRef.current&&window.turnstile)window.turnstile.render(captchaRef.current,{sitekey,callback:setCaptcha})};document.head.appendChild(script);return()=>script.remove()},[]);
  if(booking.isSuccess)return <main className="public-shell"><section className="success-card"><div className="success-icon"><CalendarDays/></div><p className="eyebrow">Tudo certo</p><h1>Horário solicitado!</h1><p>Seu pedido foi registrado. Você receberá a confirmação pelo WhatsApp.</p><Button onClick={()=>location.reload()}>Fazer outro agendamento</Button><p className="company-credit public-credit">Tecnologia por <strong>M&amp;R Solutions</strong></p></section></main>;
  return <main className="public-shell"><header className="public-header"><div className="brand-mark"><Scissors/></div><div><p>Agendamento online</p><h1>{shop.data?.name??"Barbearia"}</h1></div></header><form className="booking-card" onSubmit={(e:FormEvent)=>{e.preventDefault();booking.mutate()}}><div className="booking-step"><span>1</span><div><h2>Escolha o serviço</h2><p>Selecione o atendimento desejado.</p></div></div><div className="service-options">{services.data?.map(s=><button type="button" className={service===String(s.id)?"selected":""} key={s.id} onClick={()=>{setService(String(s.id));setSlot("")}}><span>{s.name}<small>{s.duration_minutes} minutos</small></span><strong>{money(s.price)}</strong></button>)}</div><div className="booking-step"><span>2</span><div><h2>Data e horário</h2><p>Veja os horários disponíveis.</p></div></div><input type="date" min={format(new Date(),"yyyy-MM-dd")} value={day} onChange={e=>{setDay(e.target.value);setSlot("")}}/><div className="slots">{slots.isFetching?<Loading/>:slots.data?.length?slots.data.map(s=><button type="button" className={slot===s?"selected":""} onClick={()=>setSlot(s)} key={s}>{format(new Date(s),"HH:mm")}</button>):service&&<p>Nenhum horário disponível nesta data.</p>}</div><div className="booking-step"><span>3</span><div><h2>Seus dados</h2><p>Para identificarmos o agendamento.</p></div></div><div className="form-grid"><label>Nome<input required minLength={2} value={name} onChange={e=>setName(e.target.value)}/></label><label>WhatsApp<input required minLength={10} inputMode="tel" value={whatsapp} onChange={e=>setWhatsapp(e.target.value)} placeholder="(11) 99999-9999"/></label></div><div ref={captchaRef}/><ErrorMessage error={booking.error}/><Button className="booking-submit" disabled={!service||!slot||!captcha||booking.isPending}>{booking.isPending?"Confirmando...":"Confirmar agendamento"}</Button></form><p className="company-credit public-credit">Tecnologia por <strong>M&amp;R Solutions</strong></p></main>;
}
function AdminApp() {
...
export default function App(){const parts=location.pathname.split("/").filter(Boolean);if(parts[0]==="agendar"&&parts[1])return <PublicBooking slug={parts[1]}/>;return <AdminApp/>}
